...
#if(is.null(ylim)) {
  with(subset(junk, type == "Web of Science"), plot(year, freq_scaled, type = "l", xlim = xlim, axes = FALSE, yaxs = "i", ylim = c(0, max(freq_scaled, na.rm = TRUE) * 1.1)))
#} else {
...
par(new = TRUE)
with(subset(junk, type == "Google News Archive"), plot(year, freq_scaled, axes = FALSE, type = "l", xlim = xlim, lty = 2,  yaxs = "i", ylim = c(0, max(freq_scaled, na.rm = TRUE) * 1.1)))
