...
for(i in 1:4) with(ivory_export_early_raw, lines(year, ivory_export_early_raw[,i+1] , type = "l", lty = lty.to.plot[i], col = col.stressor, lwd = 1))

#with(ivory_export_recent, lines(Year,  Total.multiplied.by.10..estimate.of.export./1e6, type = "l", lty = 3, lwd = 1.6))
with(ivory_export_recent, points(Year,  Total.multiplied.by.10..estimate.of.export./1e6, type = "p", lty = 3, lwd = .5, col = col.stressor, pch = "."))
...
with(elephant_pop_modeled, polygon(c(year, rev(year)), c(lower, rev(upper)), col = paste(col.response, "30", sep = ""), border = NA))
with(elephant_pop_modeled, lines(year, estimate, col = col.response))
with(elephant_pop, points(Year, Pop..Estimate..Number.of.elephants./1e6, cex = .7, col = col.response, pch = c(rep(21, 4), rep(19, 4))))
with(elephant_pop, segments(Year, Pop..Estimate..lower./1e6, Year, Pop..Estimate..upper./1e6, col = col.response, lwd = c(rep(0.5, 4), rep(0.8, 4))))
box(col = col.axis)
#n <- nrow(elephant_pop)
